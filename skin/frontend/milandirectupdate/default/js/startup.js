...
function updatePriceHtml()
{
    ...
    $$('p.min-price span[class="price"]').each(function(item){
        var html = $(item).innerHTML;
        if (html.search('<sup>') == -1 && html.search('</sup>') == -1) {
            var html_currency = html.sub('$', '<sup>$</sup>');
            var html_price = html_currency.sub('.', '<sup>.') + '</sup>';
    ...
    $$('p.max-price span[class="price"]').each(function(item){
        var html = $(item).innerHTML;
        if (html.search('<sup>') == -1 && html.search('</sup>') == -1) {
            var html_currency = html.sub('$', '<sup>$</sup>');
            var html_price = html_currency.sub('.', '<sup>.') + '</sup>';
    ...
    $$('p.general-price span[class="price"]').each(function(item){
        var html = $(item).innerHTML;
        if (html.search('<sup>') == -1 && html.search('</sup>') == -1) {
            var html_currency = html.sub('$', '<sup>$</sup>');
            var html_price = html_currency.sub('.', '<sup>.') + '</sup>';
...
function forgotOrderPopup()
{
    Mage.Cookies.set('order_closed', 1);
}
...
function changeDob(){
    var dateChoose = jQuery('.dob-day select').val() +'/' + jQuery('.dob-month select').val() +'/'+jQuery('.dob-year select').val();
    jQuery('.dob-full input').val(dateChoose);
}
