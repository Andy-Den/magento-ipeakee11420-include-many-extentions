...
var OneStepCheckoutLoginPopup = Class.create({
    initialize: function(options) {
        ...
        this.login_handler = function(e) {
            var dataForm = new VarienForm('onestepcheckout-login-form', true);
            if (dataForm.validator && dataForm.validator.validate()) {
                ...
                new Ajax.Request(url, {
                    ...
                    onSuccess: function(transport) {
                        ...
                        if(result.success) {
                            jQuery('.col-main').html(result.htmlupdate);
                            jQuery('.link-accounts').html(result.logged);
                            jQuery('.onestepcheckout-column-middle').removeClass('cloumn-disable');
                            jQuery('.onestepcheckout-column-left').removeClass('cloumn-disable');
                            jQuery('.onestepcheckout-column-left').addClass('hide-step-disable');
                            jQuery('.onestepcheckout-column-middle').addClass('hide-step-disable');
                            jQuery('.billing-step-disable').hide();
                            customCheckout.billingContinue();
                            customCheckout.updatePostcode();
...
function get_save_billing_function(url, set_methods_url, update_payments, triggered)
{
    ...
    return function()    {
        ...
        var totalCustoms = $$('div.grandtotal')[0];
        if(update_payments){
            var payment_methods = $$('div.payment-methods')[0];
            payment_methods.update('<div class="loading-ajax">&nbsp;</div>');
        } else {
            totalCustoms.update('<div class="loading-ajax">&nbsp;</div>');
        }
        ...
        new Ajax.Request(url, {
            ...
            if(transport.status == 200)    {
                ...
                totalCustoms.update(data.summary);
                //totals.update(data.summary);
...
function get_separate_save_methods_function(url, update_payments)
{
    ...
    return function(e)    {
        ...
        if(payment_method == 'braintreevzero'){
            jQuery('.onestepcheckout-threecolumns.checkoutcontainer .onestepcheckout-column-right .onestepcheckout-place-order-wrapper .button.btn-checkout').css({'opacity':'0.5'});
        }
        new Ajax.Request(url, {
            method: 'post',
            onSuccess: function(transport)    {
...
Billing.prototype = {
    ...
    newAddress: function(isNew){
        ...
        jQuery("#billing\\:city").prepend("<option value='' selected='selected'>Please select</option>");
    },
...
var Shipping = Class.create();
    ...
    newAddress: function(isNew){
        ...
        jQuery("#shipping\\:city").prepend("<option value='' selected='selected'>Please select</option>");
...
var customCheckout	=	function(){
    return {
        guestCheckout: function(){
            var dataForm = new VarienForm('onestepcheckout-guestcheckout-form', true);
            if (dataForm.validator && dataForm.validator.validate()) {
                jQuery('#onestepcheckout-guest-loading').show();
                jQuery('#onestepcheckout-guest-table').hide();
                guestForm = jQuery('#onestepcheckout-guestcheckout-form');
                url = guestForm.attr('action');
                new Ajax.Request(url, {
                    method: 'post',
                    parameters: guestForm.serialize(),
                    onSuccess: function(transport) {
                        jQuery('#onestepcheckout-guest-loading').hide();
                        jQuery('#onestepcheckout-guest-table').show();
                        var result = transport.responseText.evalJSON();
                        if(result.success) {
                            // Unlock step 2 update email step 2 to email of guest
                            jQuery('.onestepcheckout-column-middle').removeClass('cloumn-disable');
                            jQuery('.billing-step-disable').hide();
                            jQuery('#billing\\:email').val(jQuery('#login_guest_email').val());
                            jQuery('#billing\\:firstname').focus();
                            jQuery('.onestepcheckout-column-left').addClass('hide-step-disable');
                            jQuery('.onestepcheckout-column-middle').addClass('hide-step-disable');
                            jQuery('.onestepcheckout-column-left').addClass('cloumn-disable');
                        }
                    }.bind(this)
                });
            }
        ...
        updateUniform: function(){
            jQuery("#shipping\\:country_id").on('change', function(){
                if (jQuery('#shipping\\:region_id').css('display') == 'none') {
                    jQuery('#uniform-shipping\\:region_id').hide();
                } else {
                    jQuery('#uniform-shipping\\:region_id').show();
                }
            });
            jQuery("#billing\\:country_id").on('change', function(){
                if (jQuery('#billing\\:region_id').css('display') == 'none') {
                    jQuery('#uniform-billing\\:region_id').hide();
                } else {
                    jQuery('#uniform-billing\\:region_id').show();
                }
            });
            jQuery.uniform.update();
        },
        billingContinue: function(){
            jQuery('#onestepcheckout-continue-billing').on('click', function(){
                // TODO
                // validate -> save billing -> nextstep(unlock)
                var validateResult = true;
                jQuery('#onestepcheckout-form input, #onestepcheckout-form select, #onestepcheckout-form textarea').each(function(){
                    inputName = jQuery(this).attr('name');
                    if(typeof(inputName) !='undefined'){
                        inputName = inputName.toLowerCase();
                        if (inputName.indexOf('billing') !== -1 || inputName.indexOf('shipping') !== -1) {
                            Validation.reset(this);
                            validate = Validation.validate(this);
                            if (validate == false) {
                                validateResult = false;
                            }
                        }
                ...
                if (validateResult == true) {
                    jQuery('.payment-step-disable').hide();
                    jQuery('.onestepcheckout-column-right').removeClass('cloumn-disable');
                    jQuery('.onestepcheckout-column-middle').addClass('hide-step-disable');
                    jQuery('.onestepcheckout-column-right').addClass('hide-step-disable');
                    //jQuery('.onestepcheckout-column-middle').addClass('cloumn-disable');
                    get_save_billing_function(url_save_billing, url_set_methods, false, true)();
                    jQuery('#onestepcheckout-continue-billing').html('Update');
                }
            });
        },
        updatePostcode: function(){
            jQuery("#billing\\:postcode").on('change',function(){
                postcode    =   jQuery(this).val();
                jQuery.ajax({
                    type: "POST",
                    url: url_postcode_update,
                    data: { estimate_postcode: postcode, split: 1, type: 'billing' },
                    dataType: 'json',
                    success: function(data) {
                        jQuery("#billing-select-box").remove();
                        jQuery("#billing_address_list .input-city").replaceWith(data.suburb);
                        jQuery("#billing_address_list .input-region").replaceWith(data.state);
                        ...
                        jQuery("#uniform-billing\\:region_id").show();
                        jQuery("#billing\\:city, #billing\\:region_id").uniform({selectAutoWidth: false});
                    }
                });
            });
            jQuery("#shipping\\:postcode").on('change',function(){
                ...
                jQuery.ajax({
                    type: "POST",
                    url: url_postcode_update,
                    data: { estimate_postcode: postcode, split: 1, type: 'shipping' },
                    dataType: 'json',
                    success: function(data) {
                        jQuery("#shipping-select-box").remove();
                        jQuery("#shipping_address_list .input-city").replaceWith(data.suburb);
                        jQuery("#shipping_address_list .input-region").replaceWith(data.state);
                        ...
                        jQuery("#shipping\\:postcode").val(postcode);
                        jQuery("#uniform-shipping\\:region_id").show();
                        jQuery("#shipping\\:city, #shipping\\:region_id").uniform({selectAutoWidth: false});
