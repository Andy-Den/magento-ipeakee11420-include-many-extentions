...
Checkout.prototype = {
    initialize: function(accordion, urls){
        ...
        //this.steps = ['cart', 'billing', 'shipping', 'shipping_method', 'payment', 'review'];
        this.steps = ['cart', 'billing', 'payment'];
    ...
    {
        Effect.ScrollTo("opc-"+section);
        section = $('opc-'+section);
        section.addClassName('allow');
        this.accordion.openSection(section);
        this.showSection('cart');
        if(section == 'payment') {
            review = new Review(BASE_URL + 'checkout/onepage/saveOrder', BASE_URL + 'checkout/onepage/success');
            review.getReviewContent(BASE_URL + 'checkout/onepage/cart');
        }
    },
...
Payment.prototype = {
    ...
    initialize: function(form, saveUrl){
        ...
        this.saveUrl = saveUrl;
        //this.onSave = this.nextStep.bindAsEventListener(this);
        this.onSave = this.callSaveReview.bindAsEventListener(this);
        this.onComplete = this.resetLoadWaiting.bindAsEventListener(this);
    },
    ...
    init : function () {
        ...
        for (var i=0; i<elements.length; i++) {
            ...
            } else if (elements[i].name != 'form_key') {
    ...
    callSaveReview: function(transport){
        try{
            if(payment.lastUsedMethod == 'paypal_express'){
                window.location.href = transport.responseText.evalJSON().redirect;
            }else{
                review = new Review(BASE_URL + 'checkout/onepage/saveOrder', BASE_URL + 'checkout/onepage/success');
                review.save();
            }
...
var PaymentPal = Class.create();
PaymentPal.prototype = {
    beforeInitFunc:$H({}),
    afterInitFunc:$H({}),
    beforeValidateFunc:$H({}),
    afterValidateFunc:$H({}),
...
Review.prototype = {
    ...
    save: function(){
        ...
        //}
        if($F('biebersdorfCustomerOrderComment') != '') {
            params += '&biebersdorfCustomerOrderComment='+$F('biebersdorfCustomerOrderComment');
        }
    ...
    getReviewContent: function(link){
        jQuery('#checkout-step-cart').load(link, function() {
           updatePriceHtml();
        });
    },
