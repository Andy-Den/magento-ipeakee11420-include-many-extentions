...
Lightbox.prototype = {
    ...
    updateImageList: function() { 
        ...
        document.observe('click', (function(event){
            var target = event.findElement('a[rel^=lightbox-click]') || event.findElement('a[rel^=lightbox]') || event.findElement('area[rel^=lightbox]');
            if (target) {
                event.stop();
                this.start(target);
            }
    ...
    start: function(imageLink) {
        ...
        if ((imageLink.rel == 'lightbox')){
            ...
            if (imageLink.rel == 'lightbox[rotation]' || imageLink.rel == 'lightbox-click[rotation]')
            { 
            	this.imageArray = 
                    $$(imageLink.tagName + '[href][rel="lightbox[rotation]"]').
            ...
            // Added to show product main image first
            if((this.imageArray.length) > 1) {
            	this.imageArray.unshift(this.imageArray[(this.imageArray.length)-1]);
            	this.imageArray.pop();
            }
    ...
    removeDuplicatedImg: function(imgArray)
    {
        if (!imgArray || imgArray.length === 0)
        {
            return [];
        }
        imgArray.each(function(ele)
        {
