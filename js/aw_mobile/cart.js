...
MobileCart.prototype = {
    ...
    registerDeliveryEstimate: function (a, b) {
        this.delivery_estimate_form = $j("#" + a);
        this.delivery_estimate_form_id = a;
        this.deliveryEstimateFormSubmitUrl = b;
        this.delivery_estimate_form.submit(function (c) {
            mobileCart.postDeliveryEstimateForm(a, b);
            return false
        })
    },
    ...
    postCouponForm: function(isRemove){
        ...
        if($j('#coupon_code').val() == '') {
        	if($j('#coupon-code-error').length == 0) {
        		$j('#discount-coupon-form table').append("<tr><td align='center' colspan='2' style='color:#F00;'><div id='coupon-code-error'>Please enter coupon code</div></td></tr>")
        		$j('#discount-coupon-form table').css({border:"1px solid #f00", padding:"0 10px"});
        	}
        } else {
        	mobileCart.disableAllOverlays();
        	mobileCart.setLoader(true);
    ...
    postDeliveryEstimateForm: function (a, b) {
        
        if($j('#postcode').val() == '') {
        	if($j('#shipping-error').length == 0) {
        		$j('#shipping-zip-form table').append("<tr><td colspan='2' style='color:#F00;'><div id='shipping-error'>Please enter postcode for estimated delivery cost</div></td></tr>")
        		$j('#shipping-zip-form table').css({border:"1px solid #f00", padding:"0 10px"});
        	}
        } else {
        	mobileCart.disableAllOverlays();
            mobileCart.setLoader(true);
