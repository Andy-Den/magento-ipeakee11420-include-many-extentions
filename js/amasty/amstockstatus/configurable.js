...
//                        if ( (0 == this.options[keyCheck]['is_in_stock'] || 1 == this.options[keyCheck]['is_qnt_0']) && this.options[keyCheck]['custom_status'])
                        if (this.options[keyCheck]['custom_status'])
                        {
                            ...
                            {
                                 //settings[i].options[x].text = settings[i].options[x].text + ' (' + this.options[keyCheck]['custom_status'] + ')'; /* REF: MID-321*/
                            }
                        }
                    }
                }
...
Product.Config.prototype.configure = function(event){
    ...
    /* BEGIN REF: MID-253*/
    var options = this.config.attributes[element.config.id].options;
    for(var i=0;i<options.length;i++)
    {
        if(options[i].id == element.value)
        {
            var  image = this.config.attributes[element.config.id].options[i].images;
            var imageResize =  this.config.attributes[element.config.id].options[i].imageResize;
            if ($('image')) $('image').setAttribute("src",imageResize);
            if ($('main-image')) $('main-image').setAttribute("href",image);
            if ($('main-image-1')) $('main-image-1').setAttribute("href",image);
            var stockStatus = this.config.attributes[element.config.id].options[i].stockStatus;
            if ($('stock-status-configurable')) $('stock-status-configurable').update(stockStatus);
        }
        if (element.value == '')
        {
            if ($('stock-status-configurable')) $('stock-status-configurable').update('');
        }
