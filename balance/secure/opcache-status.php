...
if (isset($_GET['clear'])) {
    include __DIR__.'/../local/opcache-reset.php';
    return;
}
    ...
    public function getScriptStatusRows()
    {
        ...
        foreach ($dirs as $dir => $files) {
            if ($id > $limit) {
                ...
                $rows[] = "<td>...</td>";
                $rows[] = "<td>...</td>";
                $rows[] = "<td>Above are the first 100 scripts.</td>";
                $rows[] = '</tr>';
                break;
            }
            $count = count($files);
...
<html>
<head>
    <link rel="stylesheet" type="text/css" href="/balance/public/css/opcache-status.css">
...
<body>
<div id="container">
    <h1 class="title"><?php echo $dataModel->getPageTitle(); ?></h1>
...
<!--        </div>-->
    ...
    <div id="graph">
        <form>
            ...
            <span id="clearBtn" onclick="clearOpCache();">Clear</span>
        </form>
    ...
    function set_text(t) {
        if (t === "memory") {
            d3.select("#stats").html(
                "<table><tr><th style='background:#D32F2F; color:#EEE; font-size:12px; font-weight:normal;'>Used</th><td><?php echo $dataModel->getHumanUsedMemory()?></td></tr>"+
                "<tr><th style='background:#4CAF50; color:#EEE; font-size: 12px;font-weight:normal;'>Free</th><td><?php echo $dataModel->getHumanFreeMemory()?></td></tr>"+
                "<tr><th style='background:#5D4037; color:#EEE; font-size: 12px;font-weight:normal;' rowspan=\"2\">Wasted</th><td><?php echo $dataModel->getHumanWastedMemory()?></td></tr>"+
                "<tr><td><?php echo $dataModel->getWastedMemoryPercentage()?>%</td></tr></table>"
            );
        } else if (t === "keys") {
            d3.select("#stats").html(
                "<table><tr><th style='background:#D32F2F; color:#EEE; font-size:12px; font-weight:normal;'>Cached keys</th><td>"+format_value(dataset[t][0])+"</td></tr>"+
                "<tr><th style='background:#4CAF50; color:#EEE; font-size:12px; font-weight:normal;'>Free Keys</th><td>"+format_value(dataset[t][1])+"</td></tr></table>"
            );
        } else if (t === "hits") {
            d3.select("#stats").html(
                "<table><tr><th style='background:#D32F2F; color:#EEE; font-size:12px; font-weight:normal;'>Misses</th><td>"+format_value(dataset[t][0])+"</td></tr>"+
                "<tr><th style='background:#4CAF50; color:#EEE; font-size:12px; font-weight:normal;'>Cache Hits</th><td>"+format_value(dataset[t][1])+"</td></tr></table>"
            );
        } else if (t === "restarts") {
            d3.select("#stats").html(
                "<table><tr><th style='background:#D32F2F; color:#EEE; font-size:12px; font-weight:normal;'>Memory</th><td>"+dataset[t][0]+"</td></tr>"+
                "<tr><th style='background:#4CAF50; color:#EEE; font-size:12px; font-weight:normal;'>Manual</th><td>"+dataset[t][1]+"</td></tr>"+
                "<tr><th style='background:#5D4037; color:#EEE; font-size:12px; font-weight:normal;'>Keys</th><td>"+dataset[t][2]+"</td></tr></table>"
            );
        }
    }
    function change() {
...
//        });
    ...
    function clearOpCache() {
        $.ajax({
            url: '/balance/secure/opcache-status.php?clear',
            type: 'GET',
