...
//        $this->getQuote()->reserveOrderId();
        ...
        $xml .= "<ewayCardHoldersName>" . htmlspecialchars(trim($payment->getCcOwner()), ENT_QUOTES, 'UTF-8') . "</ewayCardHoldersName>";
        ...
        $xml .= "<ewayCustomerInvoiceDescription>" . htmlspecialchars(trim($invoiceDesc), ENT_QUOTES, 'UTF-8') . "</ewayCustomerInvoiceDescription>";
        $xml .= "<ewayCustomerFirstName>" . htmlspecialchars(trim($billing->getFirstname()), ENT_QUOTES, 'UTF-8') . "</ewayCustomerFirstName>";
        $xml .= "<ewayCustomerLastName>" . htmlspecialchars(trim($billing->getLastname()), ENT_QUOTES, 'UTF-8') . "</ewayCustomerLastName>";
        $xml .= "<ewayCustomerEmail>" . htmlspecialchars(trim($payment->getOrder()->getCustomerEmail()), ENT_QUOTES, 'UTF-8') . "</ewayCustomerEmail>";
        $xml .= "<ewayCustomerAddress>" . htmlspecialchars(trim($formatedAddress), ENT_QUOTES, 'UTF-8') . "</ewayCustomerAddress>";
        $xml .= "<ewayCustomerPostcode>" . htmlspecialchars(trim($billing->getPostcode()), ENT_QUOTES, 'UTF-8') . "</ewayCustomerPostcode>";
        $xml .= "<ewayCustomerInvoiceRef>" . $payment->getOrder()->getIncrementId() . "</ewayCustomerInvoiceRef>";
      //  $xml .= "<ewayCustomerInvoiceRef>" . '' . "</ewayCustomerInvoiceRef>";
$xml .= "<ewayCustomerBillingCountry>" . substr($billing->getCountry(), 0, 2) ."</ewayCustomerBillingCountry>";

        $xml .= "<ewayCustomerIPAddress>" . substr(Mage::helper('eway')->getRealIpAddr(), 0, 15) ."</ewayCustomerIPAddress>";
        if ($this->getUseccv()) {
...
//Mage::log(print_r($xml,true), null , 'debugging.log' , true);
    ...
    public function call($xml)
    {
        if ($this->getDebug()) {
            $debug = Mage::getModel('eway/api_debug')
                ->setRequestBody(preg_replace('/<ewayCardNumber>.*<\/ewayCardNumber>/', '', $xml))
                ->save();
        }
