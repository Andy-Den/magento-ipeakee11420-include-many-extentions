...
class Mage_Tax_Block_Sales_Order_Tax extends Mage_Core_Block_Template
{
    ...
    public function initTotals()
    {
        ...
        if (($this->_source->getTaxAmount() == 0) && ($this->_config->displaySalesZeroTax($store))) {
            $subtotal = (float) $this->_source->getSubtotal();
            $taxTitle = 'GST';
            $taxPercent = 11;
            if ($parent->getOrder()->getStore()->getCode() != 'australia') {
                $taxTitle = 'TAX';
                $taxPercent = 6;
            }
            ...
            $taxTotal = new Varien_Object(array(
                ...
                'value'     => $tax,
                'base_value'=> $tax,
                'label'     => $this->__('Includes: %s of', $taxTitle)
            ));
            $this->getParentBlock()->addTotal($taxTotal, 'discount');
        } elseif (!$grandTotal || ($allowTax && !$this->_config->displaySalesTaxWithGrandTotal($store))) {
            $this->_addTax();
        }
