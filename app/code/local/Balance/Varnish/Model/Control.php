 ...
 */
class Balance_Varnish_Model_Control
{
    ...
    {
        $contentTypes = array(
            self::CONTENT_TYPE_HTML  => Mage::helper('varnish')->__('HTML'),
            self::CONTENT_TYPE_CSS   => Mage::helper('varnish')->__('CSS'),
            self::CONTENT_TYPE_JS    => Mage::helper('varnish')->__('JavaScript'),
            self::CONTENT_TYPE_IMAGE => Mage::helper('varnish')->__('Images')
        );
    ...
    public function clean($domains, $urlRegEx = '.*', $contentType = '.*')
    {
        try {
            ...
            Mage::helper('varnish')->debug('Purged Varnish items with parameters ' . var_export($headers, true));
        } catch (Exception $e) {
            Mage::helper('varnish')->debug('Error during purging: ' . $e->getMessage());
    ...
    protected function _purgeCacheServers(Array $headers)
    {
        ...
        foreach ($servers as $server) {
            ...
            try {
                ...
                Mage::helper('varnish')->debug('Purging on server ' . $server . ' failed (' . $e->getMessage() . ').');
            }
        }
    }
