 ...
 */
class Balance_CampaignMonitor_UnsubscribeController extends Mage_Core_Controller_Front_Action
{
        ...
        if (isset($_GET['email'])) {
            ...
            if ($apiKey && $listID) {
                try {
                    ...
                    $result = (array)$client->GetSingleSubscriber(
                        array(
                            "ApiKey" => $apiKey,
                            "ListID" => $listID,
                            "EmailAddress" => $email
                ...
                if ($state == "Unsubscribed") {
                    try {
                        Mage::log("Balance_CampaignMonitor: Unsubscribing $email");
                        $collection = Mage::getModel('newsletter/subscriber')
                            ->loadByEmail($email)
                            ->unsubscribe();

                        Mage::getSingleton('customer/session')->addSuccess(
                            $this->__('You were successfully unsubscribed')
                        );
                    } catch (Exception $e) {
                        Mage::log("Balance_CampaignMonitor: " . $e->getMessage());
                        Mage::getSingleton('customer/session')->addError(
                            $this->__('There was an error while saving your subscription details')
                        );
                    }
                } else {
                    Mage::log(
                        "Balance_CampaignMonitor: Not unsubscribing $email, not unsubscribed in Campaign Monitor"
                    );
                }
            }
        }
