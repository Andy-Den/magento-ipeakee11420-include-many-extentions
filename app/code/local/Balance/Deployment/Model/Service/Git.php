...
class Balance_Deployment_Model_Service_Git extends Balance_Deployment_Model_Service_Abstract
{
    ...
    protected function _init()
    {
        $this->_repo = array(
            'sync' => array(
                'cmd'         =>
                    'sh ' . Mage::getModuleDir('', 'Balance_Deployment') . DS . 'shell' . DS . 'gitsync.sh '
    ...
    private function _gitpull($ip)
    {
        $ch = curl_init('http://' . $ip . '/gitsync.php');
    ...
    protected function _execute(Varien_Object $param)
    {
        $result = '';
        if ($param['cmd'] == 'sync') {
            $ips = Mage::getStoreConfig('balance_deployment/git/ips');
            $ips = explode(';', $ips);
            if (!is_array($ips) || count($ips) <= 0 || $ips == '') {
                $url = $store->getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB);
                $parse = parse_url($url);
                $ips = array($parse['host']);
            }
            foreach ($ips as $ip) {
                $result .= $this->_gitpull($ip) . "\n";
            }
        }
