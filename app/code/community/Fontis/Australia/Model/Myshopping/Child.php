...
foreach(unserialize($product_ids) as $product_id) {
    ...
    if(!empty($linkedAttributes)) {
        foreach($linkedAttributes as $la) {
            ...
            // Remove and add new fix by menhv
            //$value = $product->getResource()->getAttribute($magentoAtt)->getFrontend()->getValue($product);
            if ($product->getResource()->getAttribute($magentoAtt)){
                $value = $product->getAttributeText($magentoAtt);//$value = $product->getResource()->getAttribute($magentoAtt)->getFrontend()->getValue($product);
            }
            // end fix
            if ($value != "") {
