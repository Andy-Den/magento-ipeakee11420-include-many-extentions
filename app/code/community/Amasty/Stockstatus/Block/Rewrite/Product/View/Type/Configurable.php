        ...
        if ('product.info.options.configurable' == $this->getNameInLayout())
        {
            ...
            foreach ($allProducts as $product)
            {
                ...
                if ( (!Mage::getStoreConfig('catalog/general/displayforoutonly') || !$product->isSaleable()) || ($product->isInStock() && $stockItem->getData('qty') <= Mage::helper('amstockstatus')->getBackorderQnt() ) )
                {
                    ...
                    {
                        $stockStatus = Mage::helper('amstockstatus')->getCustomStockStatusTextConfigurable($product);
                    } elseif (Mage::helper('amstockstatus')->getCustomStockStatusTextConfigurable($product))
                    {
                        if (!$product->isInStock())
                        {
                            $stockStatus = Mage::helper('amstockstatus')->__('Out of Stock') . ' - ' . Mage::helper('amstockstatus')->getCustomStockStatusTextConfigurable($product);
                        } else 
                        {
                            $stockStatus = Mage::helper('amstockstatus')->getCustomStockStatusTextConfigurable($product);
                        }
    ...
    public function getJsonConfig()
    {
        ...
        foreach ($config['attributes'] as $key1 => $attr)
        {
            foreach ($attr['options'] as $key2 => $options)
            {
                ...
                $lightBoxConfig = Mage::getStoreConfig('easy_lightbox/general');

                if (strstr($lightBoxConfig['mainImageSize'], '_')) {
                    $mainImageSize = explode('_', $lightBoxConfig['mainImageSize'], 2);
                } else {
                    $mainImageSize = array(265, 265);
                }
                $config['attributes'][$key1]['options'][$key2]['images'] .= Mage::helper('catalog/image')->init($product, 'image');
                $config['attributes'][$key1]['options'][$key2]['imageResize'] .= Mage::helper('catalog/image')->init($product, 'image')->resize($mainImageSize[0], $mainImageSize[1]);
                // REF: END: MID-253
                ...
                $config['attributes'][$key1]['options'][$key2]['stockStatus'] .= Mage::helper('amstockstatus')->getStatusIconImage($product).$stockStatus;
                // REF: END: MID-321
