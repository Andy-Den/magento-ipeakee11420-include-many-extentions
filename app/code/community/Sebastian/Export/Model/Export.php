      ...
      if ($export_type == 'xml' || $export_type == 'csv' || $export_type == 'custom') {
        ...
        if (!$xw->openMemory()) {
          ...
          foreach ($collection as $order) {
            ...
            $shipment = $this->_getShipmentTracking($order);
                ...
                if (gettype($value) != 'array') {
                  ...
                  if ($key == 'created_at' && !empty($value)) {
                    $date = Mage::app()->getLocale()->storeDate($order->getStore(), strtotime($order->getCreatedAt()), true);
                     $xw->writeElement('created_at_time', Mage::helper('core')->formatDate($date, Mage_Core_Model_Locale::FORMAT_TYPE_MEDIUM, false));
                    $xw->writeElement('created_at_timestamp', $date->get(null, Zend_Date::TIMESTAMP));
                  }
                  if ($key == 'updated_at' && !empty($value)) {
                    $date = Mage::app()->getLocale()->storeDate($order->getStore(), strtotime($order->getCreatedAt()), true);
                    $xw->writeElement('updated_at_time', Mage::helper('core')->formatDate($date, Mage_Core_Model_Locale::FORMAT_TYPE_MEDIUM, false));
            ...
            $totalWeight = 0;
              ...
              foreach ($items as $item) {
                $totalOrderItems++;
                $totalWeight+= $item->getData('weight');
              }
            }
            ...
            if ($payment) {
              foreach ($payment->getData() as $key => $value) {
                if (gettype($value) != 'array') {
                  ...
                  // Added to split shipping amount to show per item basis
                  if($key == 'shipping_amount') {
                      $shipping_total = $value;
//					$xw->writeElement('shipping_amount', ($value/$totalOrderItems));
					continue;
                  }
//  Moved shipping amount to item data to re-calculate
            ...
            if ($items) {
              foreach ($items as $item) {
                ...
                //Mage::log($item->getData('weight'));die();
                  if($shipping_total!=0 && $totalWeight != 0)
                  {
                      $xw->writeElement('shipping_amount', round($item->getData('weight')/$totalWeight*$shipping_total, 2));
                      $xw->writeElement('grand_total', round($item->getData('weight')/$totalWeight*$shipping_total + $item->getData('price'), 2));
                  }
                  else
                  {
                      $xw->writeElement('shipping_amount', 0);
                      $xw->writeElement('grand_total', $item->getData('price'));
                  }

                if (isset($shipment[$item->getItemId()]['track_number'])) {
                    $xw->writeElement('tracking_number', $shipment[$item->getItemId()]['track_number']);
                }
                $totalqtyordered += $item->getQtyOrdered();

                // if(isset($shipment[$item->getItemId()]['date_dispatched'])) {
                // 	$xw->writeElement('dispatch_date', date('d/m/Y', strtotime($shipment[$item->getItemId()]['date_dispatched'])));
                // }
                /*
				$productObj = Mage::getModel('catalog/product')->load($item->getProductId());
          		$date = Mage::helper('amstockstatus')->showStatusWithDispatchDate($productObj);
                $xw->writeElement('dispatch_date', $date);
				*/
//                $orderDate = Mage::app()->getLocale()->storeDate($order->getStore(), strtotime($order->getCreatedAt()), true);
                if($item->getDispatchDate()){
                    $xw->writeElement('dispatch_date', $item->getDispatchDate());
//                    $product = Mage::getModel('catalog/product')->load($item->getProductId());
//                    if($product->getPreorderCalender()) {
//                         $xw->writeElement('dispatch_date', $item->getDispatchDate());
//                    } else {                        
//                         $dispatchDate = Mage::app()->getLocale()->date(Mage::helper('export')->getNextBusinessDay($orderDate))->toString('d/m/Y', 'php');                        
//                         $xw->writeElement('dispatch_date', $dispatchDate);
//                    }
                 }else{
                    $xw->writeElement('dispatch_date', $item->getDescription());
                 }

              	if($order->getData('grand_total') == $order->getData('total_paid')) {
            		$xw->writeElement('is_amount_paid', 'Yes');
                        $date = Mage::app()->getLocale()->storeDate($order->getStore(), strtotime($order->getCreatedAt()), true);
//            		$date = $orderDate;
                 //   $xw->writeElement('payment_date',  date('d/m/Y', $date->get(null, Zend_Date::TIMESTAMP)));
                    $xw->writeElement('payment_date', Mage::helper('core')->formatDate($date, Mage_Core_Model_Locale::FORMAT_TYPE_MEDIUM, false));
              	} else {
            		$xw->writeElement('is_amount_paid', 'No');
            		$xw->writeElement('payment_date', '');
              	}
...
# ftp_pasv($conn, true); # FTP: Uncomment to enable PASV mode
      ...
      if ($messages) Mage::getSingleton('adminhtml/session')->addSuccess($ordercount.' '.sprintf(Mage::helper('export')->__('orders have been exported successfully. Click here to download the export file: <a href="%s">%s</a>'), $this->getUrl('adminhtml/export_index/get')."export_id/".$exportid, Mage::helper('export')->__('Download File')));

      return $exportid;
    }

    private function _getShipmentTracking($order)
    {
    	$shipmentTracks = array();
        ...
        foreach ($shipmentCollection as $shipment){
            ...
            {
            	foreach ($item->getShipment()->getTracksCollection() as $track){
                	$shipmentTracks[$item->getOrderItemId()] = $track->getData();
                }
            }
        }
