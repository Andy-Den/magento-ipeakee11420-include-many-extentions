...
class MageWorx_Adminhtml_Seosuite_Report_CategoryController extends Mage_Adminhtml_Controller_Action {    
    ...
    protected function _prepareCategories() {
        ...
        // default store = 0
        $collection = $this->_getCategoryCollection(0);
    ...
    protected function _calculateCategories($storeId) {
        ...
        $sql = "UPDATE `seosuite_report_category` AS srp,
                    (SELECT `prepared_name`, `store_id`, COUNT(*) AS dupl_count FROM `seosuite_report_category` WHERE `store_id`=".intval($storeId)." AND `prepared_name`!='' GROUP BY `prepared_name`) AS srpr
        ...
        $sql = "UPDATE `seosuite_report_category` AS srp,
                    (SELECT `prepared_meta_title`, `store_id`, COUNT(*) AS dupl_count FROM `seosuite_report_category` WHERE `store_id`=".intval($storeId)." AND `prepared_meta_title`!='' GROUP BY `prepared_meta_title`) AS srpr
