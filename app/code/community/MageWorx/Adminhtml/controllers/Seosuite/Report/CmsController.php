...
class MageWorx_Adminhtml_Seosuite_Report_CmsController extends Mage_Adminhtml_Controller_Action {    
    ...
    protected function _getCmsCollection($storeId) {
        $collection = Mage::getResourceModel('cms/page_collection')->addStoreFilter($storeId)->addFieldToFilter('is_active', 1);       
        return $collection;
    }
    ...
    protected function _getTotalCmsCount() {
        $connection = Mage::getSingleton('core/resource')->getConnection('core_write');
        $select = $this->_getCmsCollection(0)->getSelectCountSql();
    ...
    protected function _prepareCms() {
        ...
        foreach ($collection as $item) {
            ...
            $connection->insert($tablePrefix.'seosuite_report_cms', array(
                ...
                'prepared_heading' => $helper->_prepareText($item->getContentHeading()),
                'meta_title' => $helper->_trimText($metaTitle),
                'prepared_meta_title' => $helper->_prepareText($metaTitle),
    ...
    protected function _getStores($str) {
    ...
    protected function _calculateCms($storeId) {
        ...
        $sql = "UPDATE `seosuite_report_cms` AS srp,
                    (SELECT `prepared_heading`, `store_id`, COUNT(*) AS dupl_count FROM `seosuite_report_cms` WHERE `store_id`=".intval($storeId)." AND `prepared_heading`!='' GROUP BY `prepared_heading`) AS srpr
        ...
        $sql = "UPDATE `seosuite_report_cms` AS srp,
                    (SELECT `prepared_meta_title`, `store_id`, COUNT(*) AS dupl_count FROM `seosuite_report_cms` WHERE `store_id`=".intval($storeId)." AND `prepared_meta_title`!='' GROUP BY `prepared_meta_title`) AS srpr
