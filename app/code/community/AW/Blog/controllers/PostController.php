...
class AW_Blog_PostController extends Mage_Core_Controller_Front_Action {
    ...
    public function postDetailAction(){
       $postId = $this->getRequest()->getParam('postId',null);
        if(is_null($postId)) {
            $blogData ="";
        }
        else {
            $blogData = $this->getLayout()->createBlock('blog/postcontent')->setPostId($postId)->setTemplate('aw_blog/postcontent.phtml')->toHtml();
        }
        $result = array();
        $result['content']= $blogData;
        $result['success'] =1;
        echo Mage::helper('core')->jsonEncode($result,false,array("enableJsonExprFinder"=>true));exit;
        return;
    }
