...
class AW_Searchautocomplete_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function getSearchedQuery()
    {
        ...
        if (trim($searchQuery)!="") {
           $seachQuerySynonym = Mage::getModel('catalogsearch/query')->getCollection()
                 ->addFieldToFilter('query_text',array('like'=>trim($searchQuery)))
                 ->addFieldToFilter('store_id',$currentStoreId);

           if ($seachQuerySynonym->getSize()) {
                $firstItem = $seachQuerySynonym->getFirstItem();
                if($firstItem->getSynonymFor() && $firstItem->getSynonymFor()!=""){
                    $searchQuery = $firstItem->getSynonymFor();
                }
            }
        }
