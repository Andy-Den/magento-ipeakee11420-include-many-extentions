...
class Infortis_Ultimo_Helper_Labels extends Mage_Core_Helper_Abstract
{
	...
	public function isOnSale($product)
	{
		...
		if ($specialPrice != $regularPrice)
			return $this->_nowIsBetweenOnSale($product->getData('special_from_date'), $product->getData('special_to_date'));
		else
			return false;
	}
     ...
     * MMT-154
     * */
    protected function _nowIsBetweenOnSale($fromDate, $toDate)
    {
        if ($fromDate)
        {
            $fromDate = strtotime($fromDate);
