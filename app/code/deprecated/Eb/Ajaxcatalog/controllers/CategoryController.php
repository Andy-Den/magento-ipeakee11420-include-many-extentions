...
class Eb_Ajaxcatalog_CategoryController extends Mage_Catalog_CategoryController{
     ...
     public function viewAction()
    {
        if ($category = $this->_initCatagory()) {
            ...
            /* == MMT-171 == mobile detect, just turn on on mobile */
            $detect = Mage::helper('mobiledetect');

            $isModelTablet = ($detect->isMobile() ? true : false);

            if($detect->isTablet() == 1){
                $isModelTablet = false;
            }
...
//                         echo json_encode($blocks);
                        /**
                         * This is how to return json content and make sure it can be vanrished at the same time.
                         */
                        $this->getResponse()->setHeader('Content-type', 'application/json');
                        $this->getResponse()->setBody(Mage::helper('core')->jsonEncode($blocks));
                        return;
                    }
                }
            }
